...
class CamShiftNode:
    def __init__(self):
        ...
        static_image = cv2.imread(static_file)
        ...
        while not rospy.is_shutdown():
            self.image_callback(static_image)
            rospy.sleep(0.05)
    ...
    def image_callback(self, cv_image):
        ...
        result = cv.CreateMat(cv_image.shape[0], cv_image.shape[1], cv.CV_8UC3)
        cv.Copy(cv.fromarray(cv_image), result, cv.fromarray(mask))
