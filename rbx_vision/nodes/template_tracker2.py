...
class TemplateTracker(ROS2OpenCV2):
    ...
    def process_image(self, cv_image):  
        ...
        self.detect_box = self.match_template(cv_image, self.detect_box)
    ...
    #        template_channels[0] = cv2.equalizeHist(template_channels[0])
    #        template_channels[1] = cv2.equalizeHist(template_channels[1])
    #        template_channels[2] = cv2.equalizeHist(template_channels[2])
    #        self.template = cv2.merge(template_channels)
    ...
    def match_template(self, search_image, detect_box):
        if detect_box:
            ...
            try:
                cv.Rectangle(self.marker_image, [pt1], [pt2], cv.RGB(0, 0, 255))
