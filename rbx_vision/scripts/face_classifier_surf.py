...
class FaceClassifier():
    def __init__(self):      
        ...
        if os.path.isfile(self.surf_faces_file):
            data, labels, label_map, mean, eigenvectors, face_rows, face_cols = self.load_surf_faces()
        else:
            # Load the data and compute the eigenfaces.
            data, labels, label_map, face_rows, face_cols = self.load_data_surf(image_dir)
            mean, eigenvectors = self.compute_eigenfaces(data, labels, label_map, face_rows, face_cols, n_eigenfaces)
