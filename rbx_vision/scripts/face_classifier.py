...
class FaceClassifier():
    def __init__(self):
        n_eigenfaces = 64
        self.eigenfaces_file = "eigenfaces_" + str(n_eigenfaces) + ".pkl"
        ...
        #image_dir = "/home/patrick/Downloads/lfwcrop_grey/faces"
        ...
        if os.path.isfile(self.eigenfaces_file):
            data, labels, label_map, mean, eigenvectors, face_width, face_height = self.load_eigenfaces()
        else:
            # Load the data and compute the eigenfaces.
            data, labels, label_map, face_width, face_height = self.load_data(image_dir, self.face_width, self.face_height)
            start = clock()
            mean, eigenvectors = self.compute_eigenfaces(data, labels, label_map, face_width, face_height, n_eigenfaces)
            print "PCA compute time:", 1000 * (clock() - start)
            
        # Display the mean face
        #print face_width, face_height
        cv.NamedWindow("Mean Face", cv.CV_WINDOW_NORMAL)
        cv2.imshow("Mean Face", mean.reshape(face_height, face_width))
        ...
        if show_eigenfaces:
            ...
            for i in range(eigenvectors.shape[0]):
                ...
                cv2.imshow("Eigen Face", eigen_face.reshape(face_width, face_height))
                cv2.waitKey()
...
#                print labels[s], label_map[int(label[0])]
        ...
        #test_image = cv2.imread("/home/patrick/Downloads/umist_cropped/1c/1c000.pgm", cv2.CV_LOAD_IMAGE_GRAYSCALE)


        self.reconstruct(test_image, mean, eigenvectors, face_width, face_height)
        self.classify_face(test_image, classifier, mean, eigenvectors, face_width, face_height, label_map)
...
#        print person
    ...
    def load_eigenfaces(self):
        ...
        face_width = eigenfaces['face_width']
        face_height = eigenfaces['face_height']
        return data, labels, label_map, mean, eigenvectors, face_width, face_height
            
    def compute_eigenfaces(self, data, labels, label_map, face_width, face_height, n_eigenvectors):
        ...
        result = {'data': data, 'labels': labels, 'label_map': label_map, 'mean': mean, 'eigenvectors': eigenvectors, 'face_width': face_width, 'face_height': face_height}
    ...
    def load_data(self, image_dir, face_width, face_height):
        ...
        for person in self.custom_listdir(image_dir):
            ...
            for image_file in image_files:
                ...
                #cv2.waitKey()
                image_vec = image.reshape(1, image.shape[0]*image.shape[1])
                try:
                    data_array = np.vstack((data_array, image_vec))
                except:
                    data_array = image_vec

        print n_images, "images loaded for", n_people, "different people."
        return data_array, labels, label_map, face_width, face_height
    ...
    def run_orange(self):
        ...
        kNN = knn.kNNLearner(name="kNN", k=5)
...
#            print person
#            for image_file in glob.glob(image_dir + "/" + person + "/*+20.pgm"):
#                image = cv2.imread(image_file, 0)
#                if image.shape[0] == 0 or image.shape[0] > 200:
#                    continue
#                if data_array is None:
#                    data_array = np.empty_like(image)
#                image = cv2.equalizeHist(image)        
...
#    def extract_faces(self, data, face_width, face_height):
#        for i in range(data.shape[0]):
#            image_vec = data[i]
#            image = image_vec.reshape(face_width, face_height)
...
class ANN():
    ...
    def train(self, samples, responses):
        ...
        layer_sizes = np.int32([var_n, 100, self.nclasses])
